...
void FawnForge::CGameView::Draw()
{
    ...
    Gui::EndChild();
    if ( void* pData = Gui::ReceiveDragDrop( ToString( file_type::file_type_model )))
    {
        const IMesh* pModel = m_pSystem->GetResourceManager()->LoadModel( static_cast<SFile*>( pData )->path );
    ...
    {
        const CTexture* pTexture = m_pSystem->GetResourceManager()->LoadTexture( static_cast<SFile*>( pData )->path );
        (void*) pTexture;
    }
    Gui::End();
}
