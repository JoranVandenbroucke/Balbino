...
static STSpace AvgTSpace( const STSpace* pTS0, const STSpace* pTS1 )
{
    ...
    if ( pTS0->fMagS == pTS1->fMagS && pTS0->fMagT == pTS1->fMagT && veq( pTS0->vOs, pTS1->vOs ) && veq(
            pTS0->vOt, pTS1->vOt
    ))
    {
...
static void MergeVertsFast( int piTriList_in_and_out[], STmpVert pTmpVert[], const SMikkTSpaceContext* pContext, const int iL_in, const int iR_in )
{
    ...
    else
    {
        ...
        if ( iL < iR_in )
            MergeVertsFast(
                    piTriList_in_and_out, pTmpVert, pContext, iL, iR_in
    ...
    for ( i = 0; i < iEntries; i++ )
    {
        ...
        if ( bUnassigned_A )
        {
            ...
            while ( j < iEntries && i0 == pEdges[j].i0 && i1 == pEdges[j].i1 && bNotFound )
            {
                ...
                // flip i0_B and i1_B
                GetEdge(
                        &i1_B, &i0_B, &edgenum_B, &piTriListIn[t * 3], pEdges[j].i0, pEdges[j].i1
...
static void DegenPrologue( STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris )
{
    ...
    while ( t < iNrTrianglesIn && bStillFindingGoodOnes )
    {
        ...
        else
        {
            ...
            {
                const tbool bIsGood_2 = ( pTriInfos[iNextGoodTriangleSearchIndex].iFlag & MARK_DEGENERATE ) == 0
                                        ? TTRUE
                                        : TFALSE;
                if ( bIsGood_2 )
                {
