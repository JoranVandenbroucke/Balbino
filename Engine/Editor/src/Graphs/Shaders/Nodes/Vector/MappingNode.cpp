...
CMappingNode::CMappingNode( int& id )
        ...
        id,
        {
                ...
                SSocketType{ .type=SSocketType::var_type_vector_3, .name="location", .uiName="Location" },
                SSocketType{ .type=SSocketType::var_type_vector_3, .name="rotation", .uiName="Rotation" },
                SSocketType{ .type=SSocketType::var_type_vector_3, .name="scale", .uiName="Scale" }},
...
void CMappingNode::Draw()
{
    uint64_t type{ (uint64_t) m_type };
    StartNode( m_allNames[type].c_str(), GetId());
    if ( FawnForge::Gui::DrawComboBox( "mode", type, m_allNames, {}, 100.f, true ))
    {
...
std::string CMappingNode::GetCode() const
{
    ...
    switch ( m_type )
    {
        case mapping_type_point:
            return prefix + "point" + postfix;
        case mapping_type_texture:
            return prefix + "texture" + postfix;
        case mapping_type_vector:
            return prefix + "vector" + postfix;
        case mapping_type_normal:
            return prefix + "normal" + postfix;
        case mapping_type_max:
            break;
    }
    return "";
}
