...
#include "Renderer/Imgui/backends/imgui_impl_sdl3.h"
...
void FawnForge::CInterface::Initialize( SDL_Window* pWindow, int32_t w, int32_t h, const FawnVision::CRenderer* pRenderer, ISystem* pSystem )
{
    (void) pSystem;
    m_pWindow   = pWindow;
    ...
    // CreateNew Descriptor Pool
    {
        ...
        vkCreateDescriptorPool( pRenderer->GetDevice()->GetVkDevice(), &poolInfo, nullptr, &m_descriptorPool );
    }
    
    ImGui_ImplVulkan_InitInfo info{
        .Instance        = nullptr, //todo: maybe?
        .PhysicalDevice  = pRenderer->GetDevice()->GetPhysicalDeviceInfo().device,
        .Device          = pRenderer->GetDevice()->GetVkDevice(),
        .QueueFamily     = pRenderer->GetQueue()->GetQueFamily(),
        .Queue           = pRenderer->GetQueue()->GetHandle(),
        .PipelineCache   = nullptr,
        .DescriptorPool  = m_descriptorPool,
        .Subpass         = 0,
        .MinImageCount   = pRenderer->GetSwatChain()->GetMinImage(),
        .ImageCount      = pRenderer->GetSwatChain()->GetImageCount(),
        .Allocator       = nullptr,
        .CheckVkResultFn = nullptr,
    };
    ImGui_ImplVulkan_Init( &info, pRenderer->GetRenderPass()->GetHandle());
    ...
    // Upload Fonts
    {
        ...
        const VkCommandPool   commandPool   = pRenderer->GetCommandPool()->GetHandle();
        const VkCommandBuffer commandBuffer = pRenderer->GetCommandPool()->GetCommandBuffer();
        
        vkResetCommandPool( pRenderer->GetDevice()->GetVkDevice(), commandPool, 0 );
        ...
        vkQueueSubmit( pRenderer->GetQueue()->GetHandle(), 1, &endInfo, VK_NULL_HANDLE );
        
        pRenderer->GetDevice()->WaitIdle();
...
void FawnForge::CInterface::Cleanup() const
{
    ...
    vkDestroyDescriptorPool( m_pRenderer->GetDevice()->GetVkDevice(), m_descriptorPool, nullptr );
...
void FawnForge::CInterface::ProcessEvent( SDL_Event e )
{
    ...
    switch ( e.type )
    {
        case SDL_EVENT_DROP_FILE:
        {
            ...
            m_pendingResources.emplace_back( std::string( e.drop.file ));
