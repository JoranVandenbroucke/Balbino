...
void FawnVision::CInstance::InitializeDevice( CDevice*& pDevice, uint32_t physicalDeviceIndex )
{
...
{
    for (auto & m_physicalDevice : m_physicalDevices)
    {
        for ( uint32_t j = 0; j < static_cast<uint32_t>( m_physicalDevice.queueFamilyProperties.size()); ++j )
        {
            VkBool32 supportsPresent{ VK_FALSE };
            vkGetPhysicalDeviceSurfaceSupportKHR( m_physicalDevice.device, j, surf, &supportsPresent );
            if ( supportsPresent && ( m_physicalDevice.queueFamilyProperties[j].queueFlags & VK_QUEUE_GRAPHICS_BIT ) && m_physicalDevice.deviceFeatures.samplerAnisotropy )
            {
                return j;
            }
        }
