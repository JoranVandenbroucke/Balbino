...
FawnVision::CSwapchain::CSwapchain( const CDevice* pDevice, const VkSurfaceKHR& surface )
...
void FawnVision::CSwapchain::Initialize( uint32_t width, uint32_t height )
{
    ...
    const VkSwapchainCreateInfoKHR swapChainCreateInfo{
            .sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR, .surface = m_surfaceKhr, .minImageCount = m_minImageCount, .imageFormat = m_swapSurfaceFormat.format, .imageColorSpace = m_swapSurfaceFormat.colorSpace, .imageExtent = m_swapchainExtent, .imageArrayLayers = 1, // 2 for stereo
            .imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, .imageSharingMode = VK_SHARING_MODE_EXCLUSIVE, .preTransform = m_surfaceCapabilities.currentTransform, .compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR, .presentMode = m_presentMode, .clipped = VK_TRUE, .oldSwapchain = oldSwapchain,
    };
...
{
    presentCompleteSemaphore->AddRef();
    ...
    presentCompleteSemaphore->Release();
...
const VkSwapchainKHR& FawnVision::CSwapchain::GetHandle() const
{
    return m_swapchain;
}
...
void FawnVision::CSwapchain::GetImages( std::vector<CImageResource*>& swapChainImages, uint32_t& count )
{
    ...
    for ( uint32_t i{}; i < m_imageCount; ++i )
    {
        ...
        swapChainImages[i]->InitFromSwapchain(
                m_images[i],
...
void FawnVision::CSwapchain::GetSwapExtent( uint32_t w, uint32_t h )
{
    CheckVkResult(
            vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
                    GetDevice()->GetPhysicalDeviceInfo().device, m_surfaceKhr, &m_surfaceCapabilities
            ));
...
void FawnVision::CSwapchain::GetQueueFamiliesProperties()
{
    ...
    CheckVkResult(
            vkGetPhysicalDeviceSurfaceFormatsKHR(
                    GetDevice()->GetPhysicalDeviceInfo().device, m_surfaceKhr, &formatCount, nullptr
            ));
    if ( formatCount )
    {
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(
            GetDevice()->GetPhysicalDeviceInfo().device, m_surfaceKhr, &presentModeCount, nullptr
    );
    if ( presentModeCount )
    {
        m_presentModes.resize( presentModeCount );
        vkGetPhysicalDeviceSurfacePresentModesKHR(
                GetDevice()->GetPhysicalDeviceInfo().device, m_surfaceKhr, &presentModeCount, m_presentModes.data());
    }
