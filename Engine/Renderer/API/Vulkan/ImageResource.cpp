...
FawnVision::CImageResource::CImageResource( const CDevice* pDevice )
...
void FawnVision::CImageResource::TransitionImageLayout( uint32_t mipLevels, uint32_t layers, CBuffer* command, EImageLayout::Enum newLayout )
{
    command->AddRef();
...
//            break;
    ...
    VkImageMemoryBarrier barrier{
            .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = m_imageLayout, .newLayout = static_cast<VkImageLayout>( newLayout ), .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = m_image, .subresourceRange{
                    .aspectMask = aspectMask, .baseMipLevel = 0, .levelCount = mipLevels, .baseArrayLayer = 0, .layerCount = layers,
            },
    };
...
void FawnVision::CImageResource::GenerateMipMaps( uint32_t mipLevels, CBuffer* command )
{
    command->AddRef();
    ...
    for ( uint32_t i = 1; i < mipLevels; i++ )
    {
        ...
        barrier.srcAccessMask                 = VK_ACCESS_TRANSFER_WRITE_BIT;
        barrier.dstAccessMask                 = VK_ACCESS_TRANSFER_READ_BIT;
        command->PipelineBarrier( EPipelineStageFlagBits::TransferBit, EPipelineStageFlagBits::TransferBit, barrier );
