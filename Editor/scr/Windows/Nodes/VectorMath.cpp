    ...
    switch( m_mode )
    {
        case EMode::Add:
        {
            shader = std::format( "Add({},{})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" )
                    , m_connected[1] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorB[0] ) + ", " + std::to_string( m_vectorB[1] ) + ", " + std::to_string( m_vectorB[2] ) + " )" ));
            break;
        }
        case EMode::Subtract:
        {
        ...
        case EMode::MultiplyAdd:
        {
            shader = std::format( "MultiplyAdd({},{},{})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" )
                    , m_connected[1] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorB[0] ) + ", " + std::to_string( m_vectorB[1] ) + ", " + std::to_string( m_vectorB[2] ) + " )" )
                    , m_connected[2] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorC[0] ) + ", " + std::to_string( m_vectorC[1] ) + ", " + std::to_string( m_vectorC[2] ) + " )" ));
            break;
        }
        case EMode::CrossProduct:
        {
            shader = std::format( "CrossProduct({},{})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" )
                    , m_connected[1] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorB[0] ) + ", " + std::to_string( m_vectorB[1] ) + ", " + std::to_string( m_vectorB[2] ) + " )" ));
            break;
        }
        ...
        case EMode::Length:
        {
            shader = std::format( "Length({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::Scale:
        {
            shader = std::format( "Scale({},{})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" )
                    , m_connected[1] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Float ) : std::to_string(m_fac));
            break;
        }
        case EMode::Normalize:
        {
            shader = std::format( "Normalize({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::Wrap:
        {
            shader = std::format( "Wrap({},{},{})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" )
                    , m_connected[1] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorB[0] ) + ", " + std::to_string( m_vectorB[1] ) + ", " + std::to_string( m_vectorB[2] ) + " )" )
                    , m_connected[2] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorC[0] ) + ", " + std::to_string( m_vectorC[1] ) + ", " + std::to_string( m_vectorC[2] ) + " )" ));
            break;
        ...
        case EMode::Floor:
        {
            shader = std::format( "Floor({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::Ceil:
        {
            shader = std::format( "Ceil({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::Modulo:
        {
            shader = std::format( "Modulo({},{})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" )
                    , m_connected[1] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorB[0] ) + ", " + std::to_string( m_vectorB[1] ) + ", " + std::to_string( m_vectorB[2] ) + " )" ));
            break;
        }
        case EMode::Fraction:
        {
            shader = std::format( "Fraction({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::Absolute:
        {
            shader = std::format( "Absolute({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::Minimum:
        {
            shader = std::format( "Minimum({},{})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" )
                    , m_connected[1] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorB[0] ) + ", " + std::to_string( m_vectorB[1] ) + ", " + std::to_string( m_vectorB[2] ) + " )" ));
            break;
        }
        ...
        case EMode::Sine:
        {
            shader = std::format( "Sine({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::Cosine:
        {
            shader = std::format( "Cosine({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::Tangent:
        {
            shader = std::format( "Tangent({})"
                    , m_connected[0] ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : ( "vec3( " + std::to_string( m_vectorA[0] ) + ", " + std::to_string( m_vectorA[1] ) + ", " + std::to_string( m_vectorA[2] ) + " )" ));
            break;
        }
        case EMode::MaxIndex:
