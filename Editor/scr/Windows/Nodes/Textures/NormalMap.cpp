...
std::string CNormalMap::Evaluate( std::vector<INode*>::iterator& begin, std::set<std::string>& bindings, std::set<std::string>& includes, EAttributeType::Enum attributeType )
{
    ...
    shader += std::format(
            "Normal({}, fragNormal, fragTangent, {})",
            m_connections[0]
            ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Float )
            : std::to_string( m_strength ),
            m_connections[1]
            ? ( *( begin++ ))->Evaluate( begin, bindings, includes, EAttributeType::Vector ) : "vec3(" + std::to_string( m_normal[0] ) + "," + std::to_string( m_normal[1] ) + "," + std::to_string( m_normal[2] ) + ")"
    );
...
uint8_t CNormalMap::GetVertexFlags() const
{
    return 0;
}
