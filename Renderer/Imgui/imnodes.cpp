...
namespace ImNodes
{
    namespace
    {
        enum ScopeFlags
        {
            Scope_None = 1, Scope_Editor = 1 << 1, Scope_Node = 1 << 2, Scope_Attribute = 1 << 3
        };
        
        enum AttributeType
        {
            AttributeType_None, AttributeType_Input, AttributeType_Output
        };
        ...
        struct NodeData
        {
            ...
            struct
            {
                ImU32 background, background_hovered, background_selected, outline, titlebar, titlebar_hovered, titlebar_selected;
            }      color_style;
        ...
        {
            LinkCreationType_Standard, LinkCreationType_FromDetach
        };
    ...
    namespace
    {
        ...
        inline ImVec2 eval_bezier( float t, const BezierCurve& bezier )
        {
            ...
            return ImVec2(
                    ( 1 - t ) * ( 1 - t ) * ( 1 - t ) * bezier.p0.x + 3 * ( 1 - t ) * ( 1 - t ) * t * bezier.p1.x + 3 * ( 1 - t ) * t * t * bezier.p2.x + t * t * t * bezier.p3.x,
                    ( 1 - t ) * ( 1 - t ) * ( 1 - t ) * bezier.p0.y + 3 * ( 1 - t ) * ( 1 - t ) * t * bezier.p1.y + 3 * ( 1 - t ) * t * t * bezier.p2.y + t * t * t * bezier.p3.y );
        }
        
        // Calculates the closest point along each bezier curve segment.
        ImVec2 get_closest_point_on_cubic_bezier( const int num_segments, const ImVec2& p, const BezierCurve& bezier )
        {
        ...
        inline float get_distance_to_cubic_bezier( const ImVec2& pos, const BezierCurve& bezier, const int num_segments )
        {
        ...
        inline LinkBezierData get_link_renderable( ImVec2 start, ImVec2 end, const AttributeType start_type, const float line_segments_per_length )
        {
        ...
        inline bool rectangle_overlaps_link( const ImRect& rectangle, const ImVec2& start, const ImVec2& end, const AttributeType start_type )
        {
            ...
            if ( rectangle.Overlaps( lrect ))
            {
                ...
                const LinkBezierData link_data = get_link_renderable( start, end, start_type,
    ...
    namespace
    {
        ...
        void ImDrawListSplitter_swap_channels( ImDrawListSplitter& splitter, const int lhs_idx, const int rhs_idx )
        {
            if ( lhs_idx == rhs_idx )
            {
                return;
        ...
        void draw_list_activate_click_interaction_channel()
        {
            g->canvas_draw_list->_Splitter.SetCurrentChannel( g->canvas_draw_list,
                                                              g->canvas_draw_list->_Splitter._Count - 1 );
        }
        
        void draw_list_activate_current_node_foreground()
        {
            const int foreground_channel_idx = draw_list_submission_idx_to_foreground_channel_idx(
        ...
        {
            const int submission_idx = g->node_idx_to_submission_idx.GetInt( static_cast< ImGuiID >( node_idx ), -1 );
            ...
            const int background_channel_idx = draw_list_submission_idx_to_background_channel_idx( submission_idx );
        ...
        void draw_list_swap_submission_indices( const int lhs_idx, const int rhs_idx )
        {
            ...
            const int lhs_foreground_channel_idx = draw_list_submission_idx_to_foreground_channel_idx( lhs_idx );
            const int lhs_background_channel_idx = draw_list_submission_idx_to_background_channel_idx( lhs_idx );
            const int rhs_foreground_channel_idx = draw_list_submission_idx_to_foreground_channel_idx( rhs_idx );
            const int rhs_background_channel_idx = draw_list_submission_idx_to_background_channel_idx( rhs_idx );
            
            ImDrawListSplitter_swap_channels( g->canvas_draw_list->_Splitter, lhs_background_channel_idx,
                                              rhs_background_channel_idx );
            ImDrawListSplitter_swap_channels( g->canvas_draw_list->_Splitter, lhs_foreground_channel_idx,
        ...
        ImVec2 get_screen_space_pin_coordinates( const ImRect& node_rect, const ImRect& attribute_rect, const AttributeType type )
        {
        ...
        bool mouse_in_canvas()
        {
            ...
            return is_window_hovered_or_focused && g->canvas_rect_screen_space.Contains( ImGui::GetMousePos());
        }
        ...
        void begin_link_interaction( EditorContext& editor, const int link_idx )
        {
            ...
            if ( editor.click_interaction_type == ClickInteractionType_LinkCreation )
            {
                if (( g->hovered_pin_flags & AttributeFlags_EnableLinkDetachWithDragClick ) != 0 )
                {
                    ...
                    editor.click_interaction_state.link_creation.link_creation_type = LinkCreationType_FromDetach;
                }
            }
            ...
            else
            {
                ...
                if ( modifier_pressed )
                {
                    ...
                    const int   closest_pin_idx = dist_to_start < dist_to_end ? link.start_pin_idx : link.end_pin_idx;
        ...
        {
            const bool any_ui_element_hovered = g->hovered_node_idx.has_value() || g->hovered_link_idx.has_value() || g->hovered_pin_idx.has_value() || ImGui::IsAnyItemHovered();
            
            const bool mouse_not_in_canvas = !mouse_in_canvas();
            
            if ( editor.click_interaction_type != ClickInteractionType_None || any_ui_element_hovered || mouse_not_in_canvas )
            {
                return;
            }
        ...
        void box_selector_update_selection( EditorContext& editor, ImRect box_rect )
        {
            ...
            for ( int link_idx = 0; link_idx < editor.links.pool.size(); ++link_idx )
            {
                if ( editor.links.in_use[link_idx] )
                {
                    ...
                    const ImVec2 start = get_screen_space_pin_coordinates( node_start_rect, pin_start.attribute_rect,
                                                                           pin_start.type );
                    const ImVec2 end   = get_screen_space_pin_coordinates( node_end_rect, pin_end.attribute_rect,
                                                                           pin_end.type );
        ...
        OptionalIndex find_duplicate_link( const EditorContext& editor, const int start_pin_idx, const int end_pin_idx )
        {
        ...
        bool should_link_snap_to_pin( const EditorContext& editor, const PinData& start_pin, const int hovered_pin_idx, const OptionalIndex duplicate_link )
        {
        ...
        void click_interaction_update( EditorContext& editor )
        {
            switch ( editor.click_interaction_type )
            {
                case ClickInteractionType_BoxSelection:
                {
                    ...
                    if ( g->left_mouse_released )
                    {
                        ...
                        if (( selected_idxs.Size > 0 ) && ( selected_idxs.Size < depth_stack.Size ))
                        {
                            ...
                            {
                                for ( int node_idx = depth_stack[i]; selected_idxs.contains(
                                        node_idx ); node_idx = depth_stack[i] )
                                {
                ...
                {
                    const PinData& start_pin = editor.pins.pool[editor.click_interaction_state.link_creation.start_pin_idx];
                    
                    const OptionalIndex maybe_duplicate_link_idx = g->hovered_pin_idx.has_value() ? find_duplicate_link(
                            editor, editor.click_interaction_state.link_creation.start_pin_idx,
                            g->hovered_pin_idx.value()) : OptionalIndex();
                    
                    const bool should_snap = g->hovered_pin_idx.has_value() && should_link_snap_to_pin( editor,
                                                                                                        start_pin,
                    ...
                    const bool snapping_pin_changed = editor.click_interaction_state.link_creation.end_pin_idx.has_value() && !( g->hovered_pin_idx == editor.click_interaction_state.link_creation.end_pin_idx );
                    ...
                    const ImVec2 end_pos   = should_snap
                                             ? get_screen_space_pin_coordinates( editor,
                                                                                 editor.pins.pool[g->hovered_pin_idx.value()] )
                                             : g->mouse_pos;
                    
                    const LinkBezierData link_data = get_link_renderable( start_pos, end_pos, start_pin.type,
                                                                          g->style.link_line_segments_per_length );
...
#endif
                            link_data.bezier.p0, link_data.bezier.p1, link_data.bezier.p2, link_data.bezier.p3,
                            g->style.colors[ColorStyle_Link], g->style.link_thickness, link_data.num_segments );
                    
                    const bool link_creation_on_snap = g->hovered_pin_idx.has_value() && ( editor.pins.pool[g->hovered_pin_idx.value()].flags & AttributeFlags_EnableLinkCreationOnSnap );
                    ...
                    if ( create_link && !maybe_duplicate_link_idx.has_value())
                    {
                        ...
                        if ( !g->left_mouse_released && editor.click_interaction_state.link_creation.end_pin_idx == g->hovered_pin_idx )
                        {
                            break;
                        }
        ...
        OptionalIndex resolve_hovered_pin( const ObjectPool<PinData>& pins, const ImVector<int>& occluded_pin_indices )
        {
        ...
        OptionalIndex resolve_hovered_link( const ObjectPool<LinkData>& links, const ObjectPool<PinData>& pins )
        {
            ...
            for ( int idx = 0; idx < links.pool.Size; ++idx )
            {
                ...
                const LinkBezierData link_data = get_link_renderable( start_pin.pos, end_pin.pos, start_pin.type,
                                                                      g->style.link_line_segments_per_length );
                ...
                // The distance test
                {
                    ...
                    {
                        const float distance = get_distance_to_cubic_bezier( g->mouse_pos, link_data.bezier,
        ...
        {
            const ImVec2 title_bar_height = ImVec2( 0.f,
                                                    node.title_bar_content_rect.GetHeight() + 2.0f * node.layout_style.padding.y );
        ...
        inline ImRect get_node_title_rect( const NodeData& node )
        {
            ...
            return ImRect( expanded_title_rect.Min,
                           expanded_title_rect.Min + ImVec2( node.rect.GetWidth(), 0.f ) + ImVec2( 0.f,
        ...
        void draw_grid( EditorContext& editor, const ImVec2& canvas_size )
        {
            ...
            {
                g->canvas_draw_list->AddLine( editor_space_to_screen_space( ImVec2( x, 0.0f )),
            ...
            {
                g->canvas_draw_list->AddLine( editor_space_to_screen_space( ImVec2( 0.0f, y )),
        ...
        void draw_pin_shape( const ImVec2& pin_pos, const PinData& pin, const ImU32 pin_color )
        {
            ...
            switch ( pin.shape )
            {
                ...
                {
                    g->canvas_draw_list->AddCircle( pin_pos, g->style.pin_circle_radius, pin_color, circle_num_segments,
                                                    g->style.pin_line_thickness );
                }
                    break;
                case PinShape_CircleFilled:
                {
                    g->canvas_draw_list->AddCircleFilled( pin_pos, g->style.pin_circle_radius, pin_color,
                                                          circle_num_segments );
                }
                    break;
                case PinShape_Quad:
                {
                    ...
                    g->canvas_draw_list->AddQuad( pin_pos + offset.top_left, pin_pos + offset.bottom_left,
                                                  pin_pos + offset.bottom_right, pin_pos + offset.top_right, pin_color,
                                                  g->style.pin_line_thickness );
                }
                    break;
                ...
                {
                    const QuadOffsets offset = calculate_quad_offsets( g->style.pin_quad_side_length );
                    g->canvas_draw_list->AddQuadFilled( pin_pos + offset.top_left, pin_pos + offset.bottom_left,
                                                        pin_pos + offset.bottom_right, pin_pos + offset.top_right,
                                                        pin_color );
                }
                    break;
                case PinShape_Triangle:
                {
                    ...
                    g->canvas_draw_list->AddTriangle( pin_pos + offset.top_left, pin_pos + offset.bottom_left,
                ...
                {
                    const TriangleOffsets offset = calculate_triangle_offsets( g->style.pin_triangle_side_length );
                    g->canvas_draw_list->AddTriangleFilled( pin_pos + offset.top_left, pin_pos + offset.bottom_left,
                                                            pin_pos + offset.right, pin_color );
                }
                    break;
                default:
        ...
        void draw_pin( EditorContext& editor, const int pin_idx, const bool left_mouse_clicked )
        {
            ...
            const bool pin_hovered = g->hovered_pin_idx == pin_idx && editor.click_interaction_type != ClickInteractionType_BoxSelection;
        ...
        void draw_node( EditorContext& editor, const int node_idx )
        {
            ...
            const bool node_hovered = g->hovered_node_idx == node_idx && editor.click_interaction_type != ClickInteractionType_BoxSelection;
            ...
            {
                // node base
                g->canvas_draw_list->AddRectFilled( node.rect.Min, node.rect.Max, node_background,
...
#else
                    g->canvas_draw_list->AddRectFilled( title_bar_rect.Min, title_bar_rect.Max, titlebar_background,
                                                        node.layout_style.corner_rounding,
...
#else
                    g->canvas_draw_list->AddRect( node.rect.Min, node.rect.Max, node.color_style.outline,
                                                  node.layout_style.corner_rounding, ImDrawFlags_RoundCornersAll,
                                                  node.layout_style.border_thickness );
#endif
                }
            }
        ...
        void draw_link( EditorContext& editor, const int link_idx )
        {
            ...
            const bool link_hovered = g->hovered_link_idx == link_idx && editor.click_interaction_type != ClickInteractionType_BoxSelection;
            ...
            g->canvas_draw_list->AddBezierCubic(
#endif
                    link_data.bezier.p0, link_data.bezier.p1, link_data.bezier.p2, link_data.bezier.p3, link_color,
                    g->style.link_thickness, link_data.num_segments );
        }
        
        void begin_pin_attribute( const int id, const AttributeType type, const PinShape shape, const int node_idx )
        {
    ...
    IO::LinkDetachWithModifierClick::LinkDetachWithModifierClick()
            : modifier( nullptr )
    {
    }
    ...
    Style::Style()
            : grid_spacing( 32.f ),
              node_corner_rounding( 4.f ),
              node_padding_horizontal( 8.f ),
              node_padding_vertical( 8.f ),
              node_border_thickness( 1.f ),
              link_thickness( 3.f ),
              link_line_segments_per_length( 0.1f ),
              link_hover_distance( 10.f ),
              pin_circle_radius( 4.f ),
              pin_quad_side_length( 7.f ),
              pin_triangle_side_length( 9.5 ),
              pin_line_thickness( 1.f ),
              pin_hover_radius( 10.f ),
              pin_offset( 0.f ),
              flags( StyleFlags( StyleFlags_NodeOutline | StyleFlags_GridLines )),
              colors()
    {
    }
    ...
    void BeginNodeEditor()
    {
        ...
        g->alt_mouse_clicked   = ( g->io.emulate_three_button_mouse.modifier != nullptr && *g->io.emulate_three_button_mouse.modifier && g->left_mouse_clicked ) || ImGui::IsMouseClicked(
                g->io.alt_mouse_button );
        g->left_mouse_dragging = ImGui::IsMouseDragging( 0, 0.0f );
        g->alt_mouse_dragging  = ( g->io.emulate_three_button_mouse.modifier != nullptr && g->left_mouse_dragging && ( *g->io.emulate_three_button_mouse.modifier )) || ImGui::IsMouseDragging(
                g->io.alt_mouse_button, 0.0f );
        ...
        ImGui::BeginGroup();
        {
            ...
            
            {
                ...
                g->canvas_rect_screen_space = ImRect( editor_space_to_screen_space( ImVec2( 0.f, 0.f )),
    ...
    void BeginNode( const int node_id )
    {
        ...
        node.layout_style.padding            = ImVec2( g->style.node_padding_horizontal,
                                                       g->style.node_padding_vertical );
    ...
    void Link( int id, const int start_attr_id, const int end_attr_id )
    {
        ...
        if (( editor.click_interaction_type == ClickInteractionType_LinkCreation && editor.pins.pool[link.end_pin_idx].flags & AttributeFlags_EnableLinkCreationOnSnap && editor.click_interaction_state.link_creation.start_pin_idx == link.start_pin_idx && editor.click_interaction_state.link_creation.end_pin_idx == link.end_pin_idx ) || ( editor.click_interaction_state.link_creation.start_pin_idx == link.end_pin_idx && editor.click_interaction_state.link_creation.end_pin_idx == link.start_pin_idx ))
        {
    ...
    void SetNodeScreenSpacePos( int node_id, const glm::vec2& screen_space_pos )
    {
        SetNodeScreenSpacePos( node_id, ImVec2{ screen_space_pos.x, screen_space_pos.y } );
    }
    void SetNodeScreenSpacePos( int node_id, const ImVec2& screen_space_pos )
    {
        EditorContext& editor = editor_context_get();
        NodeData     & node   = object_pool_find_or_create_object( editor.nodes, node_id );
        node.origin = screen_space_to_grid_space( editor, ImVec2{ screen_space_pos.x, screen_space_pos.y } );
    }
    ...
    bool IsLinkDropped( int* const started_at_id, const bool including_detached_links )
    {
        ...
        const bool link_dropped = ( g->element_state_change & ElementStateChange_LinkDropped ) != 0 && ( including_detached_links || editor.click_interaction_state.link_creation.link_creation_type != LinkCreationType_FromDetach );
    ...
    bool IsLinkCreated( int* started_at_node_id, int* started_at_pin_id, int* ended_at_node_id, int* ended_at_pin_id, bool* created_from_snap )
    {
        assert( g->current_scope == Scope_None );
    ...
    const char* SaveEditorStateToIniString( const EditorContext* const editor_ptr, size_t* const data_size )
    {
    ...
    void LoadEditorStateFromIniString( EditorContext* const editor_ptr, const char* const data, const size_t data_size )
    {
        if ( data_size == 0u )
        {
            return;
