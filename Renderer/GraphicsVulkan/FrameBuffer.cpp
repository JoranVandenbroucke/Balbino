...
void BalVulkan::CFrameBuffer::Initialize( const CSwapchain* pSwapchain )
{
    ...
    m_pDepthImage->Initialize( pSwapchain->GetExtend().width, pSwapchain->GetExtend().height, 1, 1, EImageLayout::Undefined, static_cast<EFormat>( attachmentDescs[ 1 ].format ), EImageUsageFlagBits::DepthStencilAttachmentBit );
