...
void BalVulkan::CFrameBuffer::Initialize( const CSwapchain* pSwapchain )
{
    ...
    const VkSubpassDependency dependency{
            ...
            VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, .srcAccessMask = 0, .dstAccessMask =
    ...
    m_pDepthImage->Initialize( pSwapchain->GetExtend().width, pSwapchain->GetExtend().height, 1, 1,
            //EImageLayout::DepthAttachmentOptimal,
            EImageLayout::Undefined, static_cast<EFormat>( attachmentDescs[ 1 ].format ), EImageUsageFlagBits::DepthStencilAttachmentBit );
