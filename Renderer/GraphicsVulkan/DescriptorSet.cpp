...
void BalVulkan::CDescriptorSet::Initialize( const CShaderPipeline* pShaderPipeline, const std::vector<SDescriptorSet>& descriptorSetsInfo )
{
    ...
    for ( const SDescriptorSet& i: descriptorSetsInfo )
    {
        ...
        {
            const auto& it = std::find_if( poolSizes.begin(), poolSizes.end(), []( const VkDescriptorPoolSize& poolSize )
            {
    ...
    const VkDescriptorPoolCreateInfo poolInfo{
            .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO, .maxSets = 3, .poolSizeCount = static_cast< uint32_t >( poolSizes.size()), .pPoolSizes = poolSizes.data(),
    };
