...
void BalVulkan::CShaderPipeline::Initialize( const std::array<CShader*, static_cast<size_t>( EShaderType::Count )>& shaders, const CFrameBuffer& frameBuffer, const std::vector<EVertexComponent>& components, uint32_t blendAttachmentSize, const CSwapchain* pSwapchain, ECullMode cullModeFlag )
{
	...
	for ( uint32_t i{ static_cast<uint32_t>( EShaderType::Vertex ) }; i < EShaderType::Count; ++i )
	{
		if ( !shaders[i] )
			continue;
		switch ( static_cast<EShaderType>( i ) )
		{
			case EShaderType::Vertex:
				shaderStages.emplace_back( VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, nullptr, 0, VK_SHADER_STAGE_VERTEX_BIT, shaders[i]->GetShaderModule(), "main", nullptr );
				break;
			case EShaderType::Geometry:
				shaderStages.emplace_back( VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, nullptr, 0, VK_SHADER_STAGE_GEOMETRY_BIT, shaders[i]->GetShaderModule(), "main", nullptr );
				break;
			case EShaderType::Fragment:
				shaderStages.emplace_back( VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, nullptr, 0, VK_SHADER_STAGE_FRAGMENT_BIT, shaders[i]->GetShaderModule(), "main", nullptr );
				break;
			default:
				continue;
		}
	...
	for ( const auto& shaderSet : m_shaderSets )
	{
		for ( auto& resource : shaderSet.second )
		{
			...
			const auto descriptorType = FindDescriptorType( resource.type, resource.mode == EShaderResourceMode::Dynamic );
	...
	CheckVkResult( vkCreateDescriptorSetLayout( GetDevice()->GetVkDevice(), &descriptorLayout, nullptr, &m_descriptorSetLayout ) );
	...
	CheckVkResult( vkCreatePipelineLayout( GetDevice()->GetVkDevice(), &pPipelineLayoutCreateInfo, nullptr, &m_pipelineLayout ), "failed to create pipeline layout!" );
	...
	const std::vector blendAttachmentStates( blendAttachmentSize, VkPipelineColorBlendAttachmentState{ .blendEnable = VK_FALSE, .colorWriteMask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT, } );
	...
	const VkPipelineMultisampleStateCreateInfo multisampleState{
		...
		.flags = 0,
		.rasterizationSamples = VK_SAMPLE_COUNT_1_BIT, //todo: msaaSamples
		.sampleShadingEnable = VK_FALSE,
	};
	const VkVertexInputBindingDescription vertexInputBindingDescription{ SVertex::GetBindingDescription() };
	const std::vector vertexInputAttributeDescriptions{ SVertex::GetAttributeDescriptions( 0, components ) };
	...
	CheckVkResult( vkCreateGraphicsPipelines( GetDevice()->GetVkDevice(), m_pipelineCache, 1, &pipelineCI, nullptr, &m_pipeline ), "failed to create graphics pipeline!" );
}
