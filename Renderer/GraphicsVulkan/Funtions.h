...
namespace BalVulkan
{
    inline bool CheckVkResult(VkResult err, std::string_view failMessage = "", VkResult exclusions = VK_SUCCESS, const std::source_location location = std::source_location::current())
    {
        ...
        std::cout << std::format( R"({}({}:{})[Vulkan] Error: VkResult = {} {} )", location.file_name(), location.line(), location.column(), location.function_name(), static_cast<uint32_t>( err ), failMessage );
        if( err < 0 )
        {
            abort();
        }
