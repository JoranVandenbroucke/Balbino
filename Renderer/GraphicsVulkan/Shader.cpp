...
void BalVulkan::CShader::Initialize( const void* pShaderCode, size_t shaderCodeSize, EShaderStage stage )
{
    ...
    switch ( stage )
    {
        ...
        case EShaderStage::FragmentShader:
            ...
            break;
        case EShaderStage::RaygenShader:
            stageBits = VK_SHADER_STAGE_RAYGEN_BIT_KHR;
            break;
        case EShaderStage::AnyHitShader:
            stageBits = VK_SHADER_STAGE_ANY_HIT_BIT_KHR;
            break;
        case EShaderStage::ClosestHitShader:
            stageBits = VK_SHADER_STAGE_ANY_HIT_BIT_KHR;
            break;
        case EShaderStage::MissShader:
            stageBits = VK_SHADER_STAGE_MISS_BIT_KHR;
            break;
        case EShaderStage::IntersectionShader:
            stageBits = VK_SHADER_STAGE_INTERSECTION_BIT_KHR;
            break;
        case EShaderStage::CallableShader:
            stageBits = VK_SHADER_STAGE_CALLABLE_BIT_KHR;
            break;
        case EShaderStage::TaskShader:
            stageBits = VK_SHADER_STAGE_TASK_BIT_NV;
            break;
        case EShaderStage::MeshShader:
    ...
    for ( const auto& stageInput : resources.stage_inputs )
    {
        ...
        const bool                      isPushConstant = reflection.get_storage_class(
                stageInput.id ) == spv::StorageClassPushConstant;
        const bool                      isBlock        = reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBlock ) || reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBufferBlock );
        const SPIRV_CROSS_NAMESPACE::ID fallbackId     = !isPushConstant && isBlock ? SPIRV_CROSS_NAMESPACE::ID(
                stageInput.base_type_id ) : SPIRV_CROSS_NAMESPACE::ID( stageInput.id );
    ...
    for ( const auto& inputAttachment : resources.subpass_inputs )
    {
        ...
        const bool                      isPushConstant = reflection.get_storage_class(
                inputAttachment.id ) == spv::StorageClassPushConstant;
        const bool                      isBlock        = reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBlock ) || reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBufferBlock );
        const SPIRV_CROSS_NAMESPACE::ID fallbackId     = !isPushConstant && isBlock ? SPIRV_CROSS_NAMESPACE::ID(
                inputAttachment.base_type_id ) : SPIRV_CROSS_NAMESPACE::ID( inputAttachment.id );
        shaderResource.name = !inputAttachment.name.empty() ? inputAttachment.name : reflection.get_fallback_name(
                fallbackId );
        //

        shaderResource.arraySize            = !spirvType.array.empty() ? spirvType.array[0] : 1;
        shaderResource.inputAttachmentIndex = reflection.get_decoration( inputAttachment.id,
                                                                         spv::DecorationInputAttachmentIndex );
        shaderResource.set                  = reflection.get_decoration( inputAttachment.id,
    ...
    for ( const auto& output : resources.stage_outputs )
    {
        ...
        const bool                      isPushConstant = reflection.get_storage_class(
                output.id ) == spv::StorageClassPushConstant;
        const bool                      isBlock        = reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBlock ) || reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBufferBlock );
        const SPIRV_CROSS_NAMESPACE::ID fallbackId     = !isPushConstant && isBlock ? SPIRV_CROSS_NAMESPACE::ID(
                output.base_type_id ) : SPIRV_CROSS_NAMESPACE::ID( output.id );
    ...
    for ( const auto& imageSampler : resources.sampled_images )
    {
        ...
        const bool                      isPushConstant = reflection.get_storage_class(
                imageSampler.id ) == spv::StorageClassPushConstant;
        const bool                      isBlock        = reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBlock ) || reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBufferBlock );
        const SPIRV_CROSS_NAMESPACE::ID fallbackId     = !isPushConstant && isBlock ? SPIRV_CROSS_NAMESPACE::ID(
                imageSampler.base_type_id ) : SPIRV_CROSS_NAMESPACE::ID( imageSampler.id );
    ...
    for ( const auto& imageStorage : resources.storage_images )
    {
        ...
        const bool                      isPushConstant = reflection.get_storage_class(
                imageStorage.id ) == spv::StorageClassPushConstant;
        const bool                      isBlock        = reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBlock ) || reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBufferBlock );
        const SPIRV_CROSS_NAMESPACE::ID fallbackId     = !isPushConstant && isBlock ? SPIRV_CROSS_NAMESPACE::ID(
                imageStorage.base_type_id ) : SPIRV_CROSS_NAMESPACE::ID( imageStorage.id );
    ...
    for ( const auto& sampler : resources.separate_samplers )
    {
        ...
        const bool                      isPushConstant = reflection.get_storage_class(
                sampler.id ) == spv::StorageClassPushConstant;
        const bool                      isBlock        = reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBlock ) || reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBufferBlock );
        const SPIRV_CROSS_NAMESPACE::ID fallbackId     = !isPushConstant && isBlock ? SPIRV_CROSS_NAMESPACE::ID(
                sampler.base_type_id ) : SPIRV_CROSS_NAMESPACE::ID( sampler.id );
    ...
    for ( const auto& uniformBuffer : resources.uniform_buffers )
    {
        ...
        const bool                      isPushConstant = reflection.get_storage_class(
                uniformBuffer.id ) == spv::StorageClassPushConstant;
        const bool                      isBlock        = reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBlock ) || reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBufferBlock );
        const SPIRV_CROSS_NAMESPACE::ID fallbackId     = !isPushConstant && isBlock ? SPIRV_CROSS_NAMESPACE::ID(
                uniformBuffer.base_type_id ) : SPIRV_CROSS_NAMESPACE::ID( uniformBuffer.id );
...
//        shaderResource.mode = (shaderResource.set == 0 && shaderResource.binding == 0)? EShaderResourceMode::Dynamic : EShaderResourceMode::Static;
    ...
    for ( const auto& storageBuffer : resources.storage_buffers )
    {
        ...
        const bool                      isPushConstant = reflection.get_storage_class(
                storageBuffer.id ) == spv::StorageClassPushConstant;
        const bool                      isBlock        = reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBlock ) || reflection.get_decoration_bitset( type.self ).get(
                spv::DecorationBufferBlock );
        const SPIRV_CROSS_NAMESPACE::ID fallbackId     = !isPushConstant && isBlock ? SPIRV_CROSS_NAMESPACE::ID(
                storageBuffer.base_type_id ) : SPIRV_CROSS_NAMESPACE::ID( storageBuffer.id );
...
VkShaderModule BalVulkan::CShader::CreateShaderModule( const std::vector<uint32_t>& data ) const
{
    ...
    CheckVkResult( vkCreateShaderModule( GetDevice()->GetVkDevice(), &createInfo, nullptr, &shaderModule ),
