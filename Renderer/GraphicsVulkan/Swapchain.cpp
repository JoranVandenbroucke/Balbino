...
{
    const VkResult result = vkAcquireNextImageKHR( GetDevice()->GetVkDevice(), m_swapchain, UINT64_MAX,
                                                   presentCompleteSemaphore->Get(), VK_NULL_HANDLE, imageIndex );
...
void BalVulkan::CSwapchain::GetImages( std::vector<CImageResource*>& swapChainImages, uint32_t& count ) const
{
    ...
    for ( uint32_t i{}; i < m_imageCount; ++i )
    {
        ...
        swapChainImages[i]->InitFromSwapchain( m_images[i], VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, m_swapchainExtent.width,
                                               m_swapchainExtent.height, m_swapSurfaceFormat.format );
    }
    count = m_imageCount;
}
...
{
    CheckVkResult(
            vkGetPhysicalDeviceSurfaceCapabilitiesKHR( GetDevice()->GetPhysicalDeviceInfo()->device, m_surfaceKhr,
                                                       &m_surfaceCapabilities ));
    ...
    m_swapchainExtent.width  = std::clamp( m_swapchainExtent.width, m_surfaceCapabilities.minImageExtent.width,
                                           m_surfaceCapabilities.maxImageExtent.width );
    m_swapchainExtent.height = std::clamp( m_swapchainExtent.height, m_surfaceCapabilities.minImageExtent.height,
...
void BalVulkan::CSwapchain::GetQueueFamiliesProperties()
{
    ...
    CheckVkResult( vkGetPhysicalDeviceSurfaceFormatsKHR( GetDevice()->GetPhysicalDeviceInfo()->device, m_surfaceKhr,
                                                         &formatCount, nullptr ));
    if ( formatCount )
    {
        m_surfaceFormats.resize( formatCount );
        CheckVkResult( vkGetPhysicalDeviceSurfaceFormatsKHR( GetDevice()->GetPhysicalDeviceInfo()->device, m_surfaceKhr,
                                                             &formatCount, m_surfaceFormats.data()));
    }
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR( GetDevice()->GetPhysicalDeviceInfo()->device, m_surfaceKhr,
