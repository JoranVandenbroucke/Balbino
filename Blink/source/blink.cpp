...
static std::filesystem::path common_path(const std::vector<std::filesystem::path>& paths)
{
	...
	for (auto it = paths.begin() + 1; it != paths.end(); ++it)
	{
		...
		for (auto it2 = file_directory.begin(), it3 = all_common_path.begin(); it2 != file_directory.end() && it3 !=
		     all_common_path.end() && *it2 == *it3; ++it2, ++it3)
		{
			common_path /= *it2;
		}
...
void blink::application::run()
{
...

	{
		...
		{
			if (std::error_code ec; _object_files[i].extension() != ".obj" || !exists(_object_files[i], ec))
				continue;
	...
	const scoped_handle watcher_handle = CreateFileW(_source_dir.native().c_str(), FILE_LIST_DIRECTORY,
	...
	while (ReadDirectoryChangesW(watcher_handle, watcher_buffer, sizeof(watcher_buffer), TRUE,
	                             FILE_NOTIFY_CHANGE_LAST_WRITE | FILE_NOTIFY_CHANGE_FILE_NAME, &size, nullptr, nullptr))
	{
		bool first_notification = true;

		// Iterate over all notification items
		for (auto info = reinterpret_cast<FILE_NOTIFY_INFORMATION*>(watcher_buffer); first_notification || info->
		     NextEntryOffset != 0;
		     first_notification = false, info = reinterpret_cast<FILE_NOTIFY_INFORMATION*>(reinterpret_cast<BYTE*>(info)
			     + info->NextEntryOffset))
		{
			...
			if (const uint64_t current_time = GetTickCount64(); _last_modifications[source_file.string()] + 3000 > current_time)
				continue;
			else
			...
			while (WaitForSingleObject(compiler_stdout, INFINITE) == WAIT_OBJECT_0 && PeekNamedPipe(
				compiler_stdout, nullptr, 0, nullptr, &size, nullptr))
			{
				std::string message(size, '\0');
				(void)ReadFile(compiler_stdout, message.data(), size, &size, nullptr);

				for (size_t offset = 0, next; (next = message.find('\n', offset)) != std::string::npos; offset = next +
				     1)
				...
				if (const size_t offset = message.find(" with code "); offset != std::string::npos)
				{
					...
					{
						call_symbol("__blink_sync", source_file.string().c_str());
...
void blink::application::read_import_address_table(const BYTE* image_base)
{
	...
	const auto import_directory_entries = reinterpret_cast<const IMAGE_IMPORT_DESCRIPTOR*>(_image_base +
	...
	for (unsigned int i = 0; import_directory_entries[i].FirstThunk != 0; i++)
	{
		...
		const auto import_name_table = reinterpret_cast<const IMAGE_THUNK_DATA*>(_image_base + import_directory_entries[
			i].Characteristics);
		const auto import_address_table = reinterpret_cast<const IMAGE_THUNK_DATA*>(_image_base +
			import_directory_entries[i].FirstThunk);
		...
		for (unsigned int k = 0; import_name_table[k].u1.AddressOfData != 0; k++)
		{
			...
			{
				const auto target_headers = reinterpret_cast<const IMAGE_NT_HEADERS*>(target_base + reinterpret_cast<
					const IMAGE_DOS_HEADER*>(target_base)->e_lfanew);
				const auto export_directory = reinterpret_cast<const IMAGE_EXPORT_DIRECTORY*>(target_base +
					target_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress);
				const auto export_name_strings = reinterpret_cast<const DWORD*>(target_base + export_directory->
					AddressOfNames);
				const auto export_name_ordinals = reinterpret_cast<const WORD*>(target_base + export_directory->
					AddressOfNameOrdinals);

				const auto ordinal = std::find(export_name_ordinals,
				                               export_name_ordinals + export_directory->NumberOfNames,
				                               IMAGE_ORDINAL(import_name_table[k].u1.Ordinal));
				if (ordinal != export_name_ordinals + export_directory->NumberOfNames)
					import_name = reinterpret_cast<const char*>(target_base + export_name_strings[std::distance(
						export_name_ordinals, ordinal)]);
				else
					continue; // Ignore ordinal imports for which the name could not be resolved
			}
			else
			{
				import_name = reinterpret_cast<const IMAGE_IMPORT_BY_NAME*>(_image_base + import_name_table[k].u1.
					AddressOfData)->Name;
			}
...
std::string blink::application::build_compile_command_line(const std::filesystem::path& source_file,
                                                           std::filesystem::path& object_file) const
{
	std::string cmdline;
	...
	if (const auto it = std::find_if(_source_files.begin(), _source_files.end(),
	                                 [&source_file](const auto& module_files)
	                                 {
		                                 return std::find_if(module_files.begin(), module_files.end(),
		                                                     [&source_file](const auto& file)
		                                                     {
			                                                     std::error_code ec;
	                                 ...
	                                 }); it != _source_files.end())
	{
		...
		if (file != INVALID_HANDLE_VALUE)
		{
			...
			(void)ReadFile(file, sections.data(), read * sizeof(IMAGE_SECTION_HEADER), &read, nullptr);
			...
			if (section != sections.end())
			{
				...
				(void)ReadFile(file, debug_data.data(), section->SizeOfRawData, &read, nullptr);
