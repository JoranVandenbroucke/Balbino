...
tbool genTangSpace( const SMikkTSpaceContext* pContext, const float fAngularThreshold )
{
    ...
    if ( pContext->m_pInterface->m_getNumFaces == NULL || pContext->m_pInterface->m_getNumVerticesOfFace == NULL || pContext->m_pInterface->m_getPosition == NULL || pContext->m_pInterface->m_getNormal == NULL
        ...
        else
        {
...

            {
                ...
                else
                {
                    ...
                    const float distSQ_02_1 = LengthSquared( vsub( P2, P0 ) );
                    const float distSQ_13_1 = LengthSquared( vsub( P3, P1 ) );

                    bQuadDiagIs_02 = distSQ_13_1 < distSQ_02_1 ? TFALSE : TTRUE;
                }
        ...
        for ( i = 0; i < pGroup->iNrFaces; i++ )// triangles
        {
            ...
            if ( !bFound )
            {
                ...
                if ( pIndices == NULL )
                {
                    // clean up and return false
                    int ss = 0;
                    for ( ss = 0; ss < iUniqueSubGroups; ss++ )
...
static void BuildNeighborsFast( STriInfo pTriInfos[], SEdge* pEdges, const int piTriListIn[], const int iNrTrianglesIn )
{
    ...
    for ( i = 0; i < iEntries; i++ )
    {
        ...
        const int ff = pEdges[ i ].f;
        tbool bUnassigned_A;

        int i0_A, i1_A;
        int edgenum_A, edgenum_B = 0;                                       // 0,1 or 2
        GetEdge( &i0_A, &i1_A, &edgenum_A, &piTriListIn[ ff * 3 ], i0, i1 );// resolve index ordering and edge_num
        bUnassigned_A = pTriInfos[ ff ].FaceNeighbors[ edgenum_A ] == -1 ? TTRUE : TFALSE;
...
static void DegenPrologue( STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris )
{
    ...
    while ( t < iNrTrianglesIn && bStillFindingGoodOnes )
    {
        ...
        else
        {
            ...
            {
                const tbool bIsGood_1 = ( pTriInfos[ iNextGoodTriangleSearchIndex ].iFlag & MARK_DEGENERATE ) == 0 ? TTRUE : TFALSE;
                if ( bIsGood_1 )
...
static void DegenEpilogue( STSpace psTspace[], STriInfo pTriInfos[], int piTriListIn[], const SMikkTSpaceContext* pContext, const int iNrTrianglesIn, const int iTotTris )
{
    ...
    for ( t = 0; t < iNrTrianglesIn; t++ )
    {
        ...
        if ( ( pTriInfos[ t ].iFlag & QUAD_ONE_DEGEN_TRI ) != 0 )
        {
            ...
            bNotFound = TTRUE;
            ii        = 0;
            while ( bNotFound && ii < 3 )
            {
                const int iVert   = pV[ ii ];
