...
void FawnForge::CAssetBrowser::Initialize(ISystem* pSystem)
{
    try
    {
        ...
        m_descriptorIcon[(uint8_t)file_type::unknown]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::unknown]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::unknown]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::folder]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::folder]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::folder]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::audio]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::audio]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::audio]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::code]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::code]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::code]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::font]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::font]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::font]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::image]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::image]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::image]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::material]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::material]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::material]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::model]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::model]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::model]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::preset]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::preset]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::preset]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::scene]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::scene]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::scene]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        m_descriptorIcon[(uint8_t)file_type::shader]
            = ImGui_ImplVulkan_AddTexture(m_pIcon[(uint8_t)file_type::shader]->GetSamplerObject()->GetSampler(),
                                          m_pIcon[(uint8_t)file_type::shader]->GetImageViewObject()->GetImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    }
...
void FawnForge::CAssetBrowser::Draw() noexcept
{
    ...
    if (m_updateCurrentDirectory)
    {
        ...
        std::ranges::sort(m_currentDirectory,
                          [](const SFile& left, const SFile& right)
                          {
    ...
    if (Gui::Begin("Asset Browser", m_isVisible, 0))
    {
        ...
        if (Gui::BeginChild("Asset File", {0, 0}, true, 1 << 6))
        {
            if (Gui::BeginChild("Icon Size Slider", {0, Gui::GetContentRegionAvail().y - 32}, false, 1 << 6))
            {
                int id{};
                bool wasOneSelected{};
                std::ranges::for_each(m_currentDirectory,
...
void FawnForge::CAssetBrowser::FindAllFiles() noexcept
{
    ...
    for (auto file = std::filesystem::recursive_directory_iterator("..\\Data"); file != std::filesystem::recursive_directory_iterator(); ++file)
    {
        ...
        // File creation
        auto filetIt = std::ranges::find_if(m_files,
                                            [&file](const SFile& filePair)
                                            {
...
{
    const auto& fileIter = std::ranges::find_if(std::as_const(m_files),
                                                [&path](const SFile& f)
                                                {
                                                    return f.path == path;
                                                });
...
void FawnForge::CAssetBrowser::CreateItem() noexcept
{
    ...
    if (Gui::StartPopup("Create New Item", true, {256, -1}))
    {
        ...
        {
            m_itemToBeCreated.fileName = "";
            m_itemToBeCreated.fileName.append(name,
                                              std::find_if(name, name + 64,
                                                           [](char c)
                                                           {
                                                               return c == '\0';
                                                           }));
    ...
    case file_type::material:
    {
        try
        {
            FawnVision::SMaterialCreateInfo createInfo{m_pPropertyPanel->GetDefaultShader(),
                                                       m_pSystem->GetResourceManager()->GetShader(m_pPropertyPanel->GetDefaultShader())->GetShaderResources()};
