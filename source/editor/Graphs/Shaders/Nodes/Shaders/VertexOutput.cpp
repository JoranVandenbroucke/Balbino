...
CVertexOutputNode::CVertexOutputNode(int& id)
    : CShaderNode{id,
                  {SSocketType{.hasEditorValues = false, .type = SSocketType::var_type::vector3, .name = "offset", .uiName = "Position Offset"},
                   ...
                   SSocketType{.hasEditorValues = false, .type = SSocketType::var_type::vector4, .name = "tangent", .uiName = "Tangent"},
                   SSocketType{.hasEditorValues = false, .type = SSocketType::var_type::vector4, .name = "uv", .uiName = "UV"},
...
{
    bool hasValue = GetInput(0)->IsConnected()
        || (std::abs(GetInput(0)->GetValue()[0]) < 1e-5f && std::abs(GetInput(0)->GetValue()[1]) < 1e-5f && std::abs(GetInput(0)->GetValue()[2]) < 1e-5f);
