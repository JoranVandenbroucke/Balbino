...
{
    const auto to   = std::ranges::find_if(m_nodes,
                                           [toNode](const CShaderNode* pNode)
                                           {
                                             return pNode->GetId() == toNode;
                                           });
    const auto from = std::ranges::find_if(m_nodes,
                                           [fromNode](const CShaderNode* pNode)
                                           {
...
{
    const std::list<CShaderNode*>::iterator to   = std::ranges::find_if(m_nodes,
                                                                        [toNode](const CShaderNode* pNode)
                                                                        {
                                                                          return pNode->GetId() == toNode;
                                                                        });
    const std::list<CShaderNode*>::iterator from = std::ranges::find_if(m_nodes,
                                                                        [fromNode](const CShaderNode* pNode)
                                                                        {
                                                                            return pNode->GetId() == fromNode;
                                                                        });
...
bool CShaderGraph::Compile(const SFile& file, int cullMode)
{
    ...
    // Sort bindings
    std::ranges::sort(shaderInfo.bindings,
                      [](const SShaderBinding& a, const SShaderBinding& b)
                      {
    ...
    for (int32_t i{shader_stage_vertex}, j{}; i < shader_stage_max; i <<= 1, ++j)
    {
        ...
        std::ranges::for_each(shaderInfo.includes,
                              [&glsCode, i](const SInclude& include)
                              {
...
std::string CShaderGraph::Compile(const CShaderNode* pNode, std::vector<const CShaderNode*>& evaluatedNodes, std::unordered_map<std::string_view, int>& outputVarCount,
                                  SShaderInfo& shaderInfo)
{
...
{
    std::stringstream ss;
    std::string arrayStr = (binding.arraySize > 1) ? std::format("[{}]", binding.arraySize)
        : binding.arraySizeString[0] != '\0'       ? std::format("[{}]", binding.arraySizeString)
                                                   : "";
