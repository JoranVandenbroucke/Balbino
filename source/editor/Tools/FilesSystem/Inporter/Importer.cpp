...
bool FawnForge::ImportFont(const std::filesystem::path& path, const char* pDestinationDirection)
{
    ...
    binaryStream << CUuid{} << static_cast<uint16_t>(FawnFiles::file_type::font) << TTF_FontLineSkip(pFont) << TTF_GetFontHinting(pFont) << TTF_FontHeight(pFont)
                 << TTF_FontAscent(pFont) << TTF_FontDescent(pFont) << TTF_FontLineSkip(pFont);
    ...
    for (uint16_t i{}; i < 256u; ++i)
    {
        ...
        binaryStream << pSurface->w << pSurface->h << pSurface->h * pSurface->pitch;
