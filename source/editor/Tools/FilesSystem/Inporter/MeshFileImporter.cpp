...
bool FawnForge::CMeshFileImporter::LoadMesh() noexcept
{
    ...
    const aiScene* scene = importer.ReadFile(m_path.string(),
                                             aiProcess_CalcTangentSpace | aiProcess_GenSmoothNormals | aiProcess_JoinIdenticalVertices | aiProcess_RemoveRedundantMaterials
                                                 | aiProcess_SplitLargeMeshes | aiProcess_Triangulate | aiProcess_GenUVCoords | aiProcess_SortByPType | aiProcess_FindDegenerates
    ...
    if (m_mergeMeshes)
    {
        std::ranges::sort(m_meshes,
                          [](const SMesh& p1, const SMesh& p2)
                          {
