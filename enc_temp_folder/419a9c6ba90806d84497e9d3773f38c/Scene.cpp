...
void Balbino::CScene::Initialize( ISystem* pSystem, uint32_t width, uint32_t height )
{
	...
	{
		SLight light;
		light.color = glm::vec3{ rand() / static_cast<float>( RAND_MAX ),rand() / static_cast<float>( RAND_MAX ),rand() / static_cast<float>( RAND_MAX ) };
		light.position = glm::vec4{ rand() / static_cast<float>( RAND_MAX ) * 20,rand() / static_cast<float>( RAND_MAX ) * 20,rand() / static_cast<float>( RAND_MAX ) * 2, 1.0f };
		light.radius = rand() / static_cast<float>( RAND_MAX ) * 10;
	...
	if ( mainCamera )
	{
		// Draw Meshed
		{
			...
			SModelObject ubo{
				...
				.proj = mainCamera->GetProjection(),
			};
			m_sLightObject.viewPos = mainCamera->GetView()[3];
			memcpy( m_pShadingBuffer, &m_sLightObject, sizeof SLightObject );
			m_pShadingBuffer->Flush();
			for ( const auto& entity : group )
			{
				...
				//m_pModelBuffer->Unmapped();

				const CMesh* pModel = static_cast<CMesh*>( m_pSystem->GetResourceManager()->GetModel( model.GetMeshId() ) );
				...
				for ( uint64_t i{}; i < size; ++i )
				{
					if ( m_pSystem->GetResourceManager()->BindMaterial( static_cast<uint64_t>( mat[i] ) ) )
						BalVulkan::DrawMesh( m_pCommandPool, metaData[i].indexStart, metaData[i].indexCount );
				}
				//m_vertexBuffer.Bind();
				//m_indexBuffer.Bind();
